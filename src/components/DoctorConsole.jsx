...
export default function DoctorConsole() {
  ...
  const [loginStep, setLoginStep] = useState(auth ? 'verified' : 'email')
  const [authStatus, setAuthStatus] = useState({
    message: 'Enter your work email to receive a login code.',
    tone: 'idle',
  })
  ...
  const handleRequestLoginCode = async (event) => {
    ...
    if (!loginEmail.trim()) {
      setAuthStatus({ message: 'Enter your work email first.', tone: 'error' })
      return
    }
    setAuthStatus({ message: 'Sending login code…', tone: 'loading' })
    try {
      const response = await fetch(`${API_BASE_URL}/api/auth/request-code`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: loginEmail, role: 'doctor' }),
      })
      const payload = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(payload.error ?? 'Unable to send login code')
      }
      setLoginStep('code')
      setAuthStatus({ message: 'Code sent. Check your inbox and enter the six digits.', tone: 'success' })
    } catch (error) {
      setAuthStatus({ message: error.message ?? 'Unable to send login code', tone: 'error' })
    }
  }
  ...
  const handleVerifyLoginCode = async (event) => {
    event.preventDefault()
    if (!loginCode.trim()) {
      setAuthStatus({ message: 'Enter the six-digit code.', tone: 'error' })
      return
    }
    setAuthStatus({ message: 'Verifying code…', tone: 'loading' })
    try {
      const response = await fetch(`${API_BASE_URL}/api/auth/verify-code`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: loginEmail, role: 'doctor', code: loginCode }),
      })
      const payload = await response.json().catch(() => ({}))
      if (!response.ok) {
        throw new Error(payload.error ?? 'Invalid login code')
      }
      setAuth(payload)
      setLoginStep('verified')
      setLoginCode('')
      setAuthStatus({ message: 'Authenticated. Load the next summary when ready.', tone: 'success' })
    } catch (error) {
      setAuthStatus({ message: error.message ?? 'Unable to verify login code', tone: 'error' })
    }
  }
    ...
    return (
      <section className="doctor-panel">
        <header>
          ...
          <h2>Log in with your work email to review AI summaries.</h2>
          <p className="subtitle">
            We send a one-time passcode to your inbox. Once verified, you will be assigned summaries from the queue.
          </p>
        </header>
        ...
        <article className="card auth-card">
          <header>
            <h3>Secure login</h3>
            <p>Codes expire after {import.meta.env.VITE_LOGIN_TTL ?? '10'} minutes.</p>
          </header>
          <form className="lookup-form" onSubmit={handleRequestLoginCode}>
          ...
          {loginStep === 'code' && (
            <form className="lookup-form" onSubmit={handleVerifyLoginCode}>
              <label className="field-label" htmlFor="doctorCode">
                6-digit code
              </label>
              <input
                id="doctorCode"
